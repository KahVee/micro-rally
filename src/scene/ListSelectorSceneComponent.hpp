...
{
public:
    ListSelectorSceneComponent(const sf::Vector2f& relativePosition, const sf::Vector2f& relativeSize, const std::string& componentClass, sf::RenderWindow& window, const sf::Color& textColor, const sf::Font& font, const sf::Color& buttonColor, const sf::Color& highlightColor, Settings* settings, int currentIndex, int listSize, int maxTextSize, std::function<std::string(float)> onChange);
